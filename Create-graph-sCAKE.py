...
for every_file in (os.listdir(data_path)):
    ...
    for sen in doc:
        ...
        #new_sen = new_sen.translate(None, string.punctuation)
        #new_sen = re.sub(hyphen_ex, ' ', new_sen)
        new_sen = re.sub(punctuation_ex, '', new_sen)
        new_sen = re.sub(roman_num_ex, '', new_sen)
        corpus.append(new_sen)
        

    ##POS tag all words forming the corpus
    tagged_words = []
    for sen in corpus:
        tokens = nltk.word_tokenize(sen)
        token_tag_tuple = nltk.pos_tag(tokens)
        tagged_words.extend(select_adj_noun(token_tag_tuple))
